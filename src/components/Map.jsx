...
const Map = ({ structure, floor, setFloor, setCenter, setDomitory, setGoldencrown, setInit }) => {
  ...
  return <div className="map">
    <Nav setSearchWarning={setSearchWarning} searching={searching} setsearching={setSearching} searchingAl={searchingAl} />
    <div className="main">
      {winWid >= 500 && <Side searching={searching} searchingAl={searchingAl}
    ...
    <div className="alert" ref={backgroundWarningRef} style={{ display: `${sizingWarning || searchWarning ? "" : "none"}` }} onClick={e => setSizingWarning(true)}>
      <div>
        ...
        {searchWarning && <>찾으시는 실이 잘못 되었거나<br />찾을 수 없습니다.</>}
        <div>
          클릭이나 Esc를 눌러 확인
        </div>
      </div>
...
const Side = ({ searching, setSearchWarning, searchingAl, setSearching, setFloor, setCenter, setDomitory, setGoldencrown, infos, winWid }) => {
  return <div className="sideleft">
    {winWid >= 500 && <form className="head" onSubmit={
      searchingAl
    }>
